...
void Drive::control_arcade(){
    float forward = deadband(controller(primary).Axis3.value(), 5);  // Axis 3 for forward/backward
    float turn = deadband(controller(primary).Axis4.value(), 5);     // Axis 4 for left/right turning
    
    DriveL.spin(fwd, to_volt(forward + turn), volt);
    DriveR.spin(fwd, to_volt(forward - turn), volt);
}
